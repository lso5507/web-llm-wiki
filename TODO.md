...
- [x] UI 재설계 (2026-06-22)
  - [x] 서비스명을 좌상단 헤더로 이동
  - [x] Hero 섹션 제거 (미니멀 디자인)
  - [x] 탭 메뉴 구조 도입 (Write / Browse / Results)
  - [x] EasyMDE 마크다운 에디터 통합 (WYSIWYG)
  - [x] 내용 입력란 확대 (500px)
  - [x] 저장 결과/인덱스를 탭으로 분리
  - [x] 자동 저장 기능 추가
  - [x] 모바일 반응형 최적화
...
- [x] 저장 버튼
- [x] 저장 결과(`status`, `summary`) 화면 표시
- [x] 마크다운 WYSIWYG 에디터 (EasyMDE)
- [x] 탭 기반 UI 구조

### 2. 문서 탐색/조회 UI
- [x] 홈 또는 별도 영역에서 최근/기본 인덱스 목록 표시
- [x] 저장 후 생성 문서 결과 확인 UI (Results 탭)
- [ ] 인덱스 검색/필터 기본 UX
- [ ] 개별 문서 상세 조회 페이지
- [ ] 문서 수정/삭제 기능
...
6. [x] 저장 후 생성 문서 결과 확인 UI를 더 풍부하게 다듬기
7. [x] 마크다운 WYSIWYG 에디터 통합
8. [x] 탭 기반 UI 구조 개편
9. [ ] 인덱스 검색/필터 기본 UX 추가
10. [ ] 파일 기반 영속화 구현
11. [ ] 개별 문서 조회 API (`GET /documents/:id`)
12. [ ] 문서 수정 API (`PUT /documents/:id`)
13. [ ] 문서 삭제 API (`DELETE /documents/:id`)
