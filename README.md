...
현재 부모 문서는 제목 포함 관계와 도메인 일치 조건으로 자동 추론한다. 제목 유사도에만 의존하기 때문에 의미적 상위 문서를 정확히 찾지 못하는 경우가 있으며, LLM이 문서 관계를 직접 판단하는 방식으로 개선이 필요하다.
...
- 저장 시 LLM이 자동 요약, 도메인 분류, `[[link]]` 제안, 의미론적 충돌 감지 수행
- 자동 저장, 도메인/태그 입력 지원
